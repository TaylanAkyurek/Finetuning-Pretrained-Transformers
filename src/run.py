...
config = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
                         n_layer=4, n_head=8, n_embd=256)

"""
Don't change above here; write your code below
"""
model = model.GPT(config).to(device)
...
if args.function == 'pretrain':
    ...
    config = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=args.pretrain_lr, lr_decay=True,
    ...
    if args.reading_params_path is not None:
        ...
        config = trainer.TrainerConfig(
            max_epochs=10, batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512 * 20,
            final_tokens=200 * len(pretrain_dataset) * block_size, num_workers=4, writer=writer
        )
    else:
        config = trainer.TrainerConfig(
            max_epochs=75, batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512 * 20,
    ...
    if args.reading_params_path is not None and os.path.isfile(args.reading_params_path):
        model.load_state_dict(torch.load(args.reading_params_path))
    else:
        print(f"File {args.reading_params_path} does not exist")
    correct = 0
    total = 0

...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_chunk = chunk[:trunc_len]

        # Prepare prefix, masked_content and suffix
        masked_len_avg = int(trunc_len / 4)

        masked_len = int(torch.randint(low=masked_len_avg // 2, high=2 * masked_len_avg, size=(1,))[0])

        mask_start = int(torch.randint(low=0, high=trunc_len - masked_len_avg + 1, size=(1,))[0])

        prefix = trunc_chunk[:mask_start]

        masked_content = trunc_chunk[mask_start:mask_start + masked_len]
        suffix = trunc_chunk[mask_start + masked_len:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string = masked_string + self.PAD_CHAR * (self.block_size - len(masked_string) + 1)

        # Create input and output tensors
        dix = torch.tensor([self.stoi[s] for s in masked_string[:-1]], dtype=torch.long)
        diy = torch.tensor([self.stoi[s] for s in masked_string[1:]], dtype=torch.long)
        return dix, diy
